/* Recipe library view: browse list (search by name) + recipe detail.
   Read-only in Phase 2 Chunk 2.3 — editing and manual entry land in Chunk 2.4.
   Default list view filters archived_at IS NULL (server-side default). */
...
(function (global) {
  ...
  function renderDetail(root, id) {
    ...
    api.recipes
      .get(id)
      .then(function (r) {
        renderDetailCard(card, r);
      })
      .catch(function (err) {
        card.innerHTML = "";
        if (err.code === "RECIPE_NOT_FOUND") {
          card.appendChild(el("div", "muted", "This recipe couldn't be found. It may have been deleted."));
        } else {
  ...
  function renderDetailCard(card, r) {
    card.innerHTML = "";

    var heading = el("h2", null, r.name);
