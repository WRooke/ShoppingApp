"""Unit tests for app/services/capture_photo.py. claude_client is mocked — no network, no
...
def test_store_and_extract_saves_file_and_calls_claude():
    with patch("app.services.capture_photo.claude_client.extract_ingredients") as mock_extract:
...
def test_store_and_extract_accepts_png():
    with patch("app.services.capture_photo.claude_client.extract_ingredients"):
